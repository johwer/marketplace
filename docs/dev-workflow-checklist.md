...
- **Visual regression baselines** (`toHaveScreenshot()`): Tests must also assert `await expect(page).toHaveScreenshot("name.png", { maxDiffPixelRatio: 0.01 })`. Baselines are stored in `__snapshots__/` next to the test file (managed by Playwright). On first run use `--update-snapshots` to generate baselines. Future test runs fail if the UI changes unexpectedly — catching regressions automatically.
...
test("WITH permission — shows element", async ({ page }) => {
    ...
    // Save screenshot for PR evidence
    await page.screenshot({ path: `${SCREENSHOT_DIR}/Component-with-feature.png` });
    // Visual regression — fails if the UI changes unexpectedly
    await expect(page).toHaveScreenshot("Component-with-feature.png", {
