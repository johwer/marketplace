...
Simulate reviewer-one' review style against a PR or ticket before he sees it. Flags must-fix issues and surfaces the clarifying questions he would ask, so they're resolved before the review cycle.
...
Extract what the PR description explicitly acknowledges. These are already "on the table" — reviewer-one may still comment but the team has a prepared answer. List them:
...
**A local `dotnet build` does not prove this.** CI builds `--configuration Release /warnaserror`,
so a nullable warning is an *error* there and silent in a default Debug build. Verify with the
configuration CI actually uses:

```bash
dotnet build services/<Svc>/<Svc>.sln --configuration Release /warnaserror
```

PROJ-3952 reported "0 errors" from Debug and CI failed on a single CS8604 — surfaced by three
separate jobs, because each one builds the same project transitively. One error can look like
three unrelated failures; read the log before concluding the branch is broadly broken.
...
For any endpoint that grants, assigns, shares or provisions something *to* someone, check both
axes. Almost every review looks at the first and almost no code checks the second:
...
Search the path the target id travels and see what validates it. If the only check is an
existence test (`ExistsAsync`, `Any(x => x.Id == id)`), the target is unconstrained: a caller
legitimately scoped to tenant A can name any id in the system and grant it something in A.

Flag it hardest when the PR *narrowed* the gate. Narrowing the actor axis is what makes the
subject axis reachable — while the action required a near-global permission, "only an admin gets
here" was doing the validating.

Concrete: PROJ-3952 gated a nurse-role grant on `PlatformAction.NurseMappingManage` and closed
the cross-retailer direction for the *grantor*, while `ValidateRoleAndSourceUser` still only
called `_userRepository.ExistsAsync(sourceUserId)`. A holder on one retailer could name any user
id, including one under a customer on another retailer, and hand them a retailer-wide platform
action. Fix shape: resolve the target's owner (`GetByIdWithCompanyAndCustomerAsync`) and require
the same customer — in the *new* path only, since the shared validator serves endpoints the PR
did not review. Return the same refusal for "no such target" and "target outside your tenant",
or the endpoint becomes an id-enumeration oracle for an authorized caller.
...
Concrete precedent in this repo, PROJ-3906: an internal service bearer named no account, so one
captured token could be used for any account. The account moved out of the URL and into a claim on
the bearer. The reviewer's framing of why that beats validating the parameter is the rule worth
keeping: with the id in both places, security depends on the far side remembering to compare them,
and a missing comparison is invisible from our side — with the id only in the claim there is
nothing to compare, so asking for somebody else's data is **"not expressible rather than merely
refused"**. PROJ-3952 applies the same shape by taking no `roleId`.

Note the companion smell: a **shared cache holding a bound capability**. That PR keeps the
account-bound token out of the singleton's five-minute cache and has a regression test asserting
it never lands there. If a diff adds a per-subject credential to an existing shared cache, flag it.

**[CACHE-KEY-SHAPE-GAP] Derived invalidation silently skips scopes that do not fit its shape**
When invalidation keys are *derived* from an entity (a `KeysFor…(entity)` helper yielding a key
per populated target column), check every scope the entity supports against the keys the helper
can actually produce. A scope whose key is not a function of one of those columns falls through
to TTL expiry, and nothing reports it.

Two things make this worth a must-fix rather than a note:
- The gap is usually documented as a *limitation* rather than a bug, so it reads as considered.
- Consequence is a write that returns 200/201 and has no effect until the TTL — which presents as
  the feature being broken, not as a cache issue.

Concrete: `AuthorizationCacheKeys.RoleAssignmentKeysForSource` yielded keys only inside
`if (assignment.TargetCompanyId …)` / `if (assignment.TargetCustomerId …)` and never yielded
`RoleAssignmentsRetailerScopedKey`. Its doc-comment excused this ("retailer-scoped assignments
cannot have their cache keys derived here"), but that key takes only `sourceUserId` — always
available. Retailer scope is also the *only* scope platform actions resolve from
(`ListPlatformActionsAsync` filters `TargetRetailer != null`), so the one scope that grants
platform actions was the one never invalidated: grant → 201 → refused for five minutes
(PROJ-3952, and the symptom documented under PROJ-3684 in `services/ServiceC/CLAUDE.md`).

When flagging, say whether the key should be yielded *conditionally*. Yielding it unconditionally
is safe but churns; gate it on the column that makes the cached value stale.

**[TEST-PINS-DEFECT] An existing test asserts the behaviour the fix must change**
When a diff fixes something and a test had to change, read the *old* test before accepting the
new one. If the old assertion encoded the defect — especially with a comment explaining why the
wrong behaviour was correct — say so in the review, because it tells you how long the bug was
load-bearing and whether anything else was built on it.

Concrete: `RoleAssignmentKeysForSource_RetailerOnlyAssignment_ReturnsGlobalAndCustomerScopesKeys`
asserted `Assert.Equal(2, keys.Count)` under the comment *"Retailer assignments can't derive
specific cache keys"* — a claim that was false. Turn such a test into a regression guard for the
opposite property rather than deleting it.
...
conditions in sequence to explain which one failed. Adding a condition to the predicate makes the
...
`bool Can…(…) => Validate(…) is null`.

The pair is fine when `Validate` only appends one generic sentence and never enumerates
conditions; that cannot drift. Concrete: PROJ-3952's `CanAssignToRetailerScope`/
`ValidateRetailerScope` duplicated three predicates while `CanAssignToScope`/`Validate` next to
it did not — reviewer-supplied inversion, adopted verbatim.

---

#### CLARIFYING QUESTIONS (reviewer-one asks before approving)
...
Output in reviewer-one' style: code-specific, no preamble, grouped by severity.
...
### Step 2 — Ask the architecture questions reviewer-one would ask
...
- Missing integration test findings: reviewer-one will always ask. Either add the test (round-trip / access scenario — NOT a unit-logic-path test, which he asks to remove), or pre-empt in the PR description with a linked follow-up ticket
