...
3. **If "Mark ready":**
   - **Screenshot-caption check (BEFORE marking ready)**: Ask the user: "Have you added screenshots to the PR description?" If yes (or if `<img`/`![` tags appear in the PR body that you didn't put there), add a caption under EACH image explaining (a) what it shows and (b) what it proves, grouped under a "Visual Verification Evidence" heading. Verify caption accuracy by opening the actual local image files (Read tool) — never caption from filenames alone; label non-evidentiary shots (mis-clicks, exploration artifacts) honestly. Use read-then-edit on the PR body, preserving the images exactly where the user placed them.
