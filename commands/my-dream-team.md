...
- Determine if there are infrastructure concerns (new migrations, Docker changes, new services)
     ...
     - **Blast-radius map (refactor tickets)**: For refactor tickets, annotate each target file with (a) has unit tests? (glob for the test file) and (b) consumer count (grep imports). Files that are **untested AND have ≥1 consumer** must be flagged for mandatory real-browser verification by the dev, and called out in the reviewer checklist as requiring browser/trace evidence — green builds prove nothing there. (Retro NOVA-3041: all 3 MUST FIX were in untested files; 11 of 18 targets had no direct test.)
...
- **Prompt:** Tell the agent:
  ...
  - **Race-paths checklist (event-driven submission tasks)**: When wiring a submit/login/save into an event handler (`onComplete`, `onChange`-driven, keyboard), enumerate EVERY code path that can call the same mutation — form `onSubmit`, Enter key, submit button, the new handler, programmatic calls — and confirm they all route through one shared concurrency guard (ref + try/finally, like `TerveystaloMfa`'s `isSubmittingRef`). Do this during implementation, not as a review finding. (Retro NOVA-3040: the OTP onComplete-vs-Enter double-submit race was only caught by Gemini's review.)
  - **Test-ergonomics checkpoint (avoid import-time side effects)**: Before placing a value at module level, ask "is this evaluated at import time in a way that forces `vi.resetModules()` + dynamic imports in tests?" Module-level reads of `window.location`, `localStorage`, env, etc. make the module stateful and break clean static test imports. Default to computing inside the component body or a lazy `useState(() => ...)` initializer. (NOVA-2951: a module-level `getRetailerFromHostname()` read forced resetModules and cost an extra refactor round.)
...
- **Prompt:** Tell the agent:
  ...
  - **Verify in the real browser**:
    ...
    4b. **Locale-display tickets: verify in a non-`sv` locale.** For any ticket whose change is about how locale-sensitive content renders (dates, numbers, translated copy, formatting), switch the app to a non-Swedish locale before capturing — `sv` is the default and hides bugs that only surface in other locales (NOVA-3200).
...
2. **Create the flag in PostHog** (team lead) using the DTF handler — idempotent, applies the team-standard config (Boolean, accept+staging hosts OR'd, 100%, active), and reuses the API key from the keychain (no re-paste):
   ...
   ~/.claude/scripts/posthog-create-flag.sh \
     --ticket "<TICKET-ID>" \
     --key "<short feature slug>"      # e.g. --ticket NOVA-2831 --key "Legacy user redirect"
